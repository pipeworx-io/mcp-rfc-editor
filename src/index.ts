...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'RFC Editor');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'rfc_text': {
      ...
      const res = await pwFetch(`${BASE}/rfc/rfc${n}.txt`, { headers: { 'User-Agent': UA } });
      if (res.status === 404) throw new Error(`RFC ${n} not found.`);
      if (!res.ok) throw await httpError(res, 'RFC Editor');
    ...
    case 'errata': {
      ...
      if (!res.ok) throw await httpError(res, 'RFC Editor errata');
...
async function loadIndex(): Promise<string> {
  ...
  const res = await pwFetch(`${BASE}/rfc-index.xml`, { headers: { 'User-Agent': UA } });
  if (!res.ok) throw await httpError(res, 'RFC index');
